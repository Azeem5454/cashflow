<x-guest-layout>
    ...
    @if(config('services.google.client_id'))
        <div class="anim-fade-up-d1 mb-5">
            ...
            <div class="relative flex items-center gap-4 py-4 mt-1">
                ...
                <span class="guest-body text-xs text-slate-400 dark:text-slate-600">or sign up with email</span>
                <div class="flex-1 h-px bg-gray-200 dark:bg-white/8"></div>
            </div>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('register') }}" class="anim-fade-up-d1 space-y-5">
        ...
        @endif
        @if(!empty($invitationRedirect))
            <input type="hidden" name="redirect" value="{{ $invitationRedirect }}">
        @endif
        ...
        {{-- Cloudflare Turnstile (invisible captcha against bot signups).
             Only renders when the site key is configured — lets local dev
             work without a Cloudflare account. --}}
        @if(config('services.turnstile.site_key'))
            ...
            @error('cf-turnstile-response')
                <p class="-mt-2 guest-body text-xs text-red-500 dark:text-red-400">{{ $message }}</p>
            @enderror
        @endif
