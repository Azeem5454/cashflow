<x-guest-layout>
    ...
    <div class="anim-fade-up mb-8">
        ...
        <h1 class="guest-display font-extrabold text-3xl text-slate-900 dark:text-white mb-2">Forgot your password?</h1>
        <p class="guest-body text-sm text-slate-500 dark:text-slate-400 leading-relaxed">No worries. Enter your email and we'll send you a secure reset link.</p>
    </div>

    {{-- Session Status (link sent) --}}
    @if (session('status'))
        <div class="anim-fade-up mb-6 flex items-start gap-3 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 rounded-xl px-4 py-4">
            <span class="flex-shrink-0 w-8 h-8 rounded-lg bg-green-100 dark:bg-green-500/15 flex items-center justify-center mt-0.5">
            ...
            <div>
                <p class="guest-body text-sm font-medium text-green-700 dark:text-green-300 mb-0.5">Reset link sent!</p>
                <p class="guest-body text-xs text-green-600 dark:text-green-400/70">{{ session('status') }}</p>
            </div>
        </div>
    @endif
