<x-guest-layout>
    ...
    {{-- Session Status --}}
    @if (session('status'))
        <div class="anim-fade-up mb-5 flex items-center gap-2.5 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 rounded-lg px-4 py-3">
            <svg class="w-4 h-4 text-green-500 dark:text-green-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="guest-body text-sm text-green-700 dark:text-green-300">{{ session('status') }}</span>
        </div>
    @endif

    {{-- Social OAuth error --}}
    @if ($errors->has('social'))
        <div class="anim-fade-up mb-5 flex items-center gap-2.5 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-lg px-4 py-3">
            <svg class="w-4 h-4 text-red-500 dark:text-red-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
            <span class="guest-body text-sm text-red-700 dark:text-red-300">{{ $errors->first('social') }}</span>
        </div>
    @endif
    ...
    @if(config('services.google.client_id'))
        <div class="anim-fade-up-d1 mb-5">
            ...
            <div class="relative flex items-center gap-4 py-4 mt-1">
                <div class="flex-1 h-px bg-gray-200 dark:bg-white/8"></div>
                <span class="guest-body text-xs text-slate-400 dark:text-slate-600">or sign in with email</span>
                <div class="flex-1 h-px bg-gray-200 dark:bg-white/8"></div>
            </div>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('login') }}" class="anim-fade-up-d1 space-y-5">
        ...
        <div>
            <div class="flex items-center justify-between mb-1.5">
                <label for="password" class="guest-body text-sm font-medium text-slate-700 dark:text-slate-300">Password</label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="guest-body text-xs text-primary dark:text-blue-light hover:text-accent transition-colors duration-150">
                        Forgot password?
                    </a>
                @endif
            </div>
        ...
        <div class="flex items-center">
            <label class="flex items-center gap-2.5 cursor-pointer select-none">
                ...
                <span class="guest-body text-sm text-slate-500 dark:text-slate-400">Remember me for 30 days</span>
            </label>
        </div>
        ...
        <div class="relative flex items-center gap-4 py-1">
            <div class="flex-1 h-px bg-gray-200 dark:bg-white/8"></div>
            <span class="guest-body text-xs text-slate-400 dark:text-slate-600">New here?</span>
            <div class="flex-1 h-px bg-gray-200 dark:bg-white/8"></div>
        </div>
