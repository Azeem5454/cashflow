<div class="p-4 sm:p-8 dark:text-white text-gray-900">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="font-display font-extrabold text-2xl dark:text-white text-gray-900 tracking-tight">Subscriptions</h1>
                ...
                @forelse($subscriptions as $sub)
                    @php
                        $statusColor = match($sub->stripe_status) {
                            ...
                            default    => 'dark:bg-slate-800 dark:text-slate-400 bg-gray-100 text-gray-500',
                        };
                    @endphp
                        ...
                        <td class="px-5 py-3.5">
                            @if($sub->owner)
                                <a href="{{ route('admin.users.show', $sub->owner) }}" wire:navigate
                                   class="dark:text-white text-gray-900 dark:hover:text-blue-light hover:text-primary font-body transition-colors">
                                    {{ $sub->owner->name }}
                                </a>
